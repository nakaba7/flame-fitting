...
HOST_B = "nakaba7-b.local"  # Raspberry Pi 3(口周辺カメラ) - IPv6アドレス
...
def main(args):
    image_num = args.n
    if args.c == 'both':
        with socket.socket(socket.AF_INET6, socket.SOCK_STREAM) as sa:
            with socket.socket(socket.AF_INET6, socket.SOCK_STREAM) as sb:
