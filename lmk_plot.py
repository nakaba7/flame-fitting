...
def plot_3d(landmarks_3d, index=False):
    ...
    ax1.scatter(landmarks_3d[:, 0], landmarks_3d[:, 1], landmarks_3d[:, 2], s=20, c='r')
    if index:
...
def plot_3lmk(landmark_2d, landmark_3d, target_3d):
    ...
    ax1.scatter(landmark_2d[:, 0], landmark_2d[:, 1], s=20, c='r', label='2D Landmarks')
...
def plot_2d_3d_compare(landmarks_2d, landmarks_3d):
    ...
    ax1.scatter(landmarks_2d[:, 0], landmarks_2d[:, 1], s=20, c='r')
    ...
    ax2.scatter(landmarks_3d[:, 0], landmarks_3d[:, 1], landmarks_3d[:, 2], s=20, c='r')
    for i, txt in enumerate(range(landmarks_3d.shape[0])):
...
def plot_3d_3d_samecoords(landmarks_3d_1, landmarks_3d_2):
    """
    2つの3次元ランドマークを同じ座標系で比較する関数.
    """
    fig = plt.figure(figsize=(14, 7))
    
    # First set of 3D landmarks
    ax1 = fig.add_subplot(121, projection='3d')
    ax1.scatter(landmarks_3d_1[:, 0], landmarks_3d_1[:, 1], landmarks_3d_1[:, 2], color='blue', label='True')
    ax1.scatter(landmarks_3d_2[:, 0], landmarks_3d_2[:, 1], landmarks_3d_2[:, 2], color='red', label='Predicted')
    #for i, point in enumerate(landmarks_3d_1):
    #   ax1.text(point[0], point[1], point[2], str(i), color='red', fontsize=9)
    ax1.set_xlabel('X')
    ax1.set_ylabel('Y')
...
if __name__ == "__main__":
    input3ddir1 = glob.glob('output_landmark/estimated_3d/test/*.npy')
    input3ddir2 = glob.glob('output_landmark/estimated_3d/*.npy')

    for i in range(0, len(input3ddir2)):
        print(input3ddir1[i])
        print(input3ddir2[i])
        landmarks_3d_1 = np.load(input3ddir1[i])
        landmarks_3d_2 = np.load(input3ddir2[i])
        plot_3d_3d_samecoords(landmarks_3d_1, landmarks_3d_2)
